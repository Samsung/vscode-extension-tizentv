...
module.exports = async function launchApplication(isDebug, wgtFilePath) {
  ...
  if (isDebug) {
    ...
    controller.addStep({
      ...
      items: ['Run On TV', 'Run On TV Emulator', 'Run On TV Simulator'].map(
        label => ({ label })
      )
    });
  }
  ...
  if (wgtFilePath) {
    ...
    if (configFile && configFile.length > 0) {
      xml2js.parseString(configFile[0].data.toString(), (error, result) => {
        if (error) {
    ...
    webApp = TVWebApp.openProject(workspace.uri.fsPath);
    if (!webApp) {
  ...
  switch (results[0]) {
    ...
    case 'Run On TV':
      ...
      portForward = configUtil.getConfig(configUtil.PORT_FORWARD);

      webApp.logInfo(`Target IP :: ${targetIP}`);
      try {
        await webApp.launchOnTV(targetIP, chromeExecPath, isDebug, portForward);
        logInfo('Application launched');
        vscode.window.showInformationMessage(
          `Launched '${webApp.id}.${webApp.name}' on ${targetIP}.`
        );
      } catch (error) {
