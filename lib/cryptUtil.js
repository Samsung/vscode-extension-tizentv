...
function encryptPassword(password, pwdFile) {
    logger.log(`crypt util: encrypt password`);
    if (platform == 'win32') {
        logger.log('crypt util: win32 platform');
        try {
            execSync(`${cryptTool} --encrypt "${password}" ${pwdFile}`);
        } catch (err) {
            if (err.stderr && err.stderr.toString()) {
                logger.log('crypt util: ${err.stderr.toString()}');
            }
        }
    } else if (platform == 'linux') {
        logger.log('crypt util: linux platform');
        execSync(
            `${cryptTool} store --label="tizen-studio" -p "${password}" keyfile ${pwdFile} tool certificate-manager`
        );
    } else if (platform == 'darwin') {
        logger.log('crypt util: darwin platform');
        execSync(
...
function decryptPassword(pwdFile) {
    logger.log('crypt util: decrypt Password');
    let password = '';
    if (platform == 'win32') {
        logger.log('crypt util: win32 platform');
        try {
        ...
        out = out.toString();
        if (out) {
            logger.log(`out: ${out}, length: ${out.length}`);
            out.trim();
...
function checkP12Password(file, password) {
    logger.log('crypt util: check P12 Password');
    try {
        ...
        logger.log("crypt util: ${err.message}");
        return false;
    }
...
function parseP12File(p12File, password) {
    logger.log("crypt util: parse P12 File");
    let p12Content = {
        privateKey: '',
        certChain: []
    };
