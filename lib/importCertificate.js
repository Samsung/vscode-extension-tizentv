...
function checkCertificateDir(certDir) {
    logger.log('importCertificate: checkCertificateDir(${certDir})');
	...
	if (!fs.existsSync(certDir)) {
		logger.log('importCertificate: Certificate file not found');
...
function checkCertficatePassword(password) {
    logger.log('importCertificate: checkCertficatePassword(${password})');
	if (cryptUtil.checkP12Password(certFileTemp, password)) {
		return null;
	} else {
		logger.log('importCertificate: password is incorrect');
		return 'The password you entered is incorrect.';
	}
}

function importAuthorCertificate(controller, totalSteps, step) {
	logger.log('importCertificate: start importAuthorCertificate');
...
function importDistributorCertificate(controller, totalSteps, step) {
	logger.log('importCertificate: start importDistributorCertificate');
	let folderBtn = StepController.FileBroswer;
	folderBtn.bindAction(async function(thisInput) {
		let folder = await vscode.window.showOpenDialog({
			canSelectFiles: true,
...
module.exports = async function importCertificate() {
	logger.log('importCertificate: start importCertificate');
