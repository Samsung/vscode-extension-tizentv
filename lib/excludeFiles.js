...
module.exports = function excludeFiles(uri) {
    ...
    if (null == excludeFiles || '' == excludeFiles) {
        ...
        for (i = 0; i < exFileArr.length; i++) {
            if (
                exFileArr[i] == uri.fsPath ||
                exFileArr[i] == path.dirname(uri.fsPath)
            ) {
    ...
    vscode.workspace
        ...
        .update(configUtil.EXCLUDE_FILES, excludeFiles, false);
    vscode.window.showInformationMessage(
        uri.fsPath + ' is excluded successful!'
    );
};
