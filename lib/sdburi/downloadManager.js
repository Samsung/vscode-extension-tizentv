...
function getPackageInfo(rootURL, packageName, options) {
  ...
  let pkgList = rootURL + `/pkg_list_${platformNameAndBit}`;
...
async function downloadPkgFromPath(packagePath, options) {
  ...
  if (packagePath === undefined || packagePath === '') {
    console.error(
      moduleName +
        'downloadManager.downloadPkgFromPath(): packagePath is invaild!'
    );
    return;
  }
  makeFilePath(tmpDir);
  ...
  try {
    console.log(
      '[webide-common-tizentv]Downloading from ${packagePath} to ${tmpDir}${path.sep}${pkgName}'
    );
    await pipeline(
