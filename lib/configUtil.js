...
const excludeFiles = 'excludeFiles';
...
async function configFromUserInput(config) {
  return new Promise((resolve, reject) => {
    if (!vscode.workspace.getConfiguration('tizentv').has(config)) {
      logger.log('configUtil : Invalid configuration item');
...
module.exports = {
  ...
  getConfig: config => vscode.workspace.getConfiguration('tizentv').get(config),
  ...
  checkTVConnection: async () => {
    ...
    do {
      if (host == null) {
        ...
        await vscode.workspace
          .getConfiguration('tizentv')
          .update(hostPCAddress, host, true);
      }
      ...
      if (target == null) {
        ...
        await vscode.workspace
          .getConfiguration('tizentv')
          .update(targetDeviceAddress, target, true);
      }
      ...
      if (select == 'Change Host IP Address') {
        logger.log('configUtil[checkTVConnection] : Change Host IP Address');
        host = await configFromUserInput(hostPCAddress);
        await vscode.workspace
          .getConfiguration('tizentv')
          .update(hostPCAddress, host, true);
      } else if (select == 'Change Target IP Address') {
        logger.log('configUtil[checkTVConnection] : Change Target IP Address');
        target = await configFromUserInput(targetDeviceAddress);
        await vscode.workspace
          .getConfiguration('tizentv')
          .update(targetDeviceAddress, target, true);
