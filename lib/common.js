...
const tizenStudioDataPath = join(homedir(), 'tizen-studio-data', 'vscode-tizentv');
...
async function runCommandInTerminal() {
  ...
  terminal.show(true);
  terminal.sendText(`mkdir -p ${resourcePath} && mkdir -p ${authorPath} && cp ${profileTemplate} ${resourcePath};`);
...
async function checkTizenStudioDataDirectory() {
  try {
    ...
    // await checkTizenStudioDataDirectory();
  }
}
