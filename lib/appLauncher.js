...
const sdbExec = path.resolve(extensionRootPath, 'tools', sdbToolname);
    ...
    openChromeDevTool() {
        ...
        httpGet(`http://${ipAddr}:${port}/json`, (res) => {
            ...
            res.on('end', () => {
                ...
                this._execAsync(`${this.chromeExec} --new-window --enable-blink-features=ShadowDOMV0,CustomElementsV0,HTMLImports http://${ipAddr}:${port}${devUrl}`);
            })
        })
    }
